...
class Group(GroupCreate):
    ...
    recentActivityScore: float = Field(default=0.0, ge=0)
    fillingUpFast: bool = False
...
def normalize_group_record(raw_group: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    meeting_type = pick('meetingType', 'meeting_type')
    time_slot = pick('timeSlot', 'time_slot')

    study_style_raw = pick('studyStyle', default=[]) or []
    if isinstance(study_style_raw, str):
        try:
            study_style = json.loads(study_style_raw)
        except json.JSONDecodeError:
            study_style = [item.strip() for item in study_style_raw.split(',') if item.strip()]
    elif isinstance(study_style_raw, list):
        study_style = study_style_raw
    else:
        study_style = list(study_style_raw)

    group_size = pick('groupSize', 'group_size')
    ...
    last_activity_raw = pick('lastActivity', 'last_activity', 'lastActivityAt')
    created_at_iso = normalize_ts(created_at_raw)
    last_activity_iso = normalize_ts(last_activity_raw) or created_at_iso
    ...
    last_activity_dt = parse_iso(last_activity_iso)
    created_at_dt = parse_iso(created_at_iso)

    fill_ratio = (member_count / max_members) if max_members else 0
    ...
    filling_up_fast_value = pick('fillingUpFast')
    if filling_up_fast_value is None:
        filling_up_fast_value = fill_ratio >= 0.75
    else:
        filling_up_fast_value = coerce_bool(filling_up_fast_value)

    starts_soon_value = pick('startsSoon')
    if starts_soon_value is None and created_at_dt:
        starts_soon_value = (now - created_at_dt).total_seconds() <= 7 * 24 * 3600
    elif starts_soon_value is None:
        starts_soon_value = False
    else:
        starts_soon_value = coerce_bool(starts_soon_value)

    recent_score = pick('recentActivityScore')
    ...
    if recent_score is None:
        if last_activity_dt:
            hours_since = max((now - last_activity_dt).total_seconds() / 3600, 0)
            freshness = max(0.0, 1.0 - min(hours_since / 168, 1.0))
            recent_score = round(freshness * 3 + fill_ratio * 2, 3)
        else:
            recent_score = 0.0
    ...
    normalized = {
        ...
        'recentActivityScore': recent_score,
        'fillingUpFast': bool(filling_up_fast_value),
        'startsSoon': bool(starts_soon_value),
    }
...
async def google_oauth_callback(request: Request, payload: GoogleCallbackRequest):
    """
    ...
    try:
        ...
        # UTDF requires usage of create_or_update_oauth_user in local_db.py
...
async def refresh_access_token(request: Request, refresh_request: RefreshTokenRequest):
    ...
    try:
        ...
        if async_initialized and user_repo:
            if hasattr(user_repo, "get_user_by_id"):
...
async def get_current_user_profile(request: Request, current_user = Depends(get_current_user)):
    ...
    try:
        ...
        if async_initialized and user_repo:
            if hasattr(user_repo, "get_user_by_id"):
                user_record = await user_repo.get_user_by_id(current_user["user_id"])
            else:
                user_record = await user_repo.get_user_by_google_id(current_user["user_id"])
        else:
            user_record = db.get_user_by_id(current_user["user_id"])
...
async def update_current_user_profile(
    ...
    try:
        ...
        if async_initialized and user_repo:
            if hasattr(user_repo, "update_user_by_id"):
                updated_user = await user_repo.update_user_by_id(
                    user_id=current_user["user_id"],
                    user_data={
                        'name': sanitized_name,
                ...
                # Fallback to updating by google_id if repository doesn't support ID-based update
            ...
            # Local DB uses google_id for upsert; use the one from user_record
...
async def create_study_group(
    ...
    try:
        ...
        def sanitize_optional(value: Optional[str], max_length: int = 60) -> Optional[str]:
            if value is None:
                return None
            cleaned = sanitize_string(value, max_length=max_length)
            return cleaned or None

        sanitized_department = sanitize_optional(group_data.department)
        sanitized_difficulty = sanitize_optional(group_data.difficulty)
        sanitized_meeting_type = sanitize_optional(group_data.meetingType)
        sanitized_time_slot = sanitize_optional(group_data.timeSlot)
        sanitized_group_size = sanitize_optional(group_data.groupSize)

        sanitized_study_styles: List[str] = []
        for style in (group_data.studyStyle or [])[:6]:
            cleaned_style = sanitize_string(style, max_length=40)
            if cleaned_style:
                sanitized_study_styles.append(cleaned_style)
...
async def get_trending_groups_endpoint(request: Request, limit: int = 6):
    """Return trending study groups based on recent activity and growth."""
    try:
        safe_limit = max(1, min(20, limit))
        groups_data = db.get_trending_groups(safe_limit)
        normalized = [normalize_group_record(group) for group in groups_data]
        return [Group(**group) for group in normalized if group]
    except Exception as exc:
        logger.error("Failed to load trending groups", error=str(exc))
        raise HTTPException(status_code=500, detail="Failed to load trending groups")
