...
class Database:
    """Synchronous SQLite-backed database used by background tasks.

    Note: The main API uses app.data.local_db for most operations. This module is
    kept for components that rely on dataclass shapes (e.g., background tasks).
    """
    ...
    def _get_connection(self):
        if self.db_type == "sqlite":
            ...
            conn.row_factory = sqlite3.Row
            return conn
        # Placeholder for optional DynamoDB support
        import boto3
    ...
    def update_user(self, user_id: str, updates: Dict[str, Any]) -> Optional[User]:
        if self.db_type == "sqlite":
            ...
            cursor.execute(
                f"UPDATE users SET {', '.join(set_clauses)} WHERE userId = ?",
                values,
            )
            conn.commit()
            conn.close()
    ...
    def create_group(self, group: Group) -> Group:
        if self.db_type == "sqlite":
            conn = self._get_connection()
            cursor = conn.cursor()
            created_at = group.created_at or datetime.utcnow().replace(tzinfo=timezone.utc).isoformat()
            last_activity = group.lastActivity or created_at
            cursor.execute(
                """
                INSERT INTO groups (
                    groupId,
                ...
                (
                    ...
                    group.meetingType,
                    group.timeSlot,
                    json.dumps(group.studyStyle or []),
                    group.groupSize,
                    created_at,
                    last_activity,
                ),
